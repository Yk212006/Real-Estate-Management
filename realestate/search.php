...
if(isset($_POST['submit'])){
    // Sanitize inputs
    $search_value = trim(mysqli_real_escape_string($con, $_POST['search'] ?? ''));
    $delivery_type = mysqli_real_escape_string($con, $_POST['delivery_type'] ?? '');
    $search_price = intval($_POST['search_price'] ?? 0);
    $property_type = mysqli_real_escape_string($con, $_POST['property_type'] ?? '');

    // Validate required fields
    if(empty($search_value) || empty($search_price) || empty($delivery_type) || empty($property_type)){
    ...
    $base_query = "SELECT * FROM properties 
                  WHERE (property_title LIKE '%$search_value%' 
                         OR property_details LIKE '%$search_value%' 
                         OR property_address LIKE '%$search_value%' 
                         OR property_type LIKE '%$search_value%')
    ...
    switch($search_price) {
        case 1:
            $query = $base_query . " AND (price BETWEEN 5000 AND 50000)";
            break;
        case 2:
            $query = $base_query . " AND (price BETWEEN 50000 AND 100000)";
            break;
        case 3:
            $query = $base_query . " AND (price BETWEEN 100000 AND 200000)";
            break;
        case 4:
            $query = $base_query . " AND (price >= 200000)";
            break;
        default:
            // If no valid price range selected, redirect to index
            header("Location: index.php");
            exit();
    }

    // Execute query if we have a valid one
    if(!empty($query)) {
        ...
        if(!$result) {
            ...
            error_log("Search query failed: " . mysqli_error($con));
    ...
    $('#newsletterForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                setTimeout(function() { messageDiv.fadeOut('slow', function() { $(this).html('').show(); }); }, 5000);
            },
            error: function() {
...
<!-- Mirrored from thebootstrapthemes.com/live/thebootstrapthemes-realestate/search.php by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 11 Apr 2017 02:44:51 GMT -->
</html>
